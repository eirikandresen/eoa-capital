...
def fetch_yahoo_osefx(start_date):
    try:
        start = int(datetime.strptime(start_date, '%Y-%m-%d').timestamp())
        end   = int((datetime.now() + timedelta(days=2)).timestamp())
        ...
        for t, c in zip(timestamps, closes):
            ...
            val = round(float(c), 4)
            if 100 < val < 2500:
                osefx[datetime.utcfromtimestamp(t).strftime('%Y-%m-%d')] = val
        print(f'Yahoo OSEFX: got {len(osefx)} rows')
        return osefx
    except Exception as e:
...
def main():
    ...
    # Clean up bad index values
    fixed = 0
    for row in existing:
        if row.get('o') and (row['o'] < 100 or row['o'] > 2500):
            print(f'Fixing bad index value {row["o"]} on {row["d"]}')
            row['o'] = None
            fixed += 1
    if fixed:
    ...
    osefx_map = fetch_yahoo_osefx(fetch_from)
    if not osefx_map:
